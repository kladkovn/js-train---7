...
function createSymbolWithDescription(property) {
  Symbol(property);// Створюємо символ symbol з описом property
  
  return property;//Повертаємо опис символа symbol
}
...
function createSymbolProperty(property) {
  let sum = Symbol();// Створимо змінну sym яка дорівнює символу без опису
  let obj = {};// Створимо пустий об'єкт
  obj[sum] = property;// Додамо властивість до об'єкту, ключем якої буде наш символ, а значенням буде аргумент property
...
function createAndCompareTwoSymbols(value) {
  ...
  return console.log(symbol1 === symbol2);  // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
}
...
function createSymbolForKey(key) {
  Symbol.for(key);// Використовуємо Symbol.for() для створення нового символу symbol зі значенням key
  ...
  return Symbol.for(key);// Повертаємо створений символ
}
...
function useSymbolToStringTag() {
  ...
  return (myObject[Symbol] = "CustomObject");// Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
